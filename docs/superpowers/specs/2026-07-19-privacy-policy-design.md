...
选择“单构建产物内的路径分发”方案：`main.tsx` 根据 `window.location.pathname` 渲染首页或隐私页。它能提供 `/privacy` 的稳定直达链接，复用既有 React、语言偏好和设计令牌，并避免新增路由依赖。Vite 静态托管环境须将未知路径回退至 `index.html`；GitHub Pages 的部署配置若不支持该回退，应在发布前将 `/privacy` 映射到相同入口。
